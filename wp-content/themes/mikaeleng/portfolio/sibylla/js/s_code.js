...
var s_account="sonyericssoncampdev"
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,3gp"
s.linkInternalFilters="javascript:,sonyericsson.com"
...
function s_doPlugins(s) {
	...
	/*Set country*/
	if(!s.prop13){
		s.prop13=s.getQueryParam('cc');
	}
	
	/*Set location*/
	if(!s.prop12){
		s.prop12=s.getQueryParam('cc')+'_'+s.getQueryParam('lc');
		
		if(!s.getQueryParam('cc')&&!s.getQueryParam('lc')){
				s.prop12="query string not set";
			}
	}
	...
	/* Copy props to eVars*/
	if(s.prop12&&!s.eVar11) s.eVar11=s.prop12;
	if(s.prop13&&!s.eVar10) s.eVar10=s.prop13;
	if(s.prop14&&!s.eVar12) s.eVar12=s.prop14;
	
	/* Plugin: detectFlash v0.6-H*/ 
	s.prop10=s.getFlash('s_fv');

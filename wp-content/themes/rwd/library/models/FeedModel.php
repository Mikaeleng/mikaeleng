<?php
if ((strpos($_SERVER['SERVER_NAME'], 'engbo') === false)) {
...
function get_wall_feed_model($args)
{
    ...
    while ($custom_query->have_posts()) : $custom_query->the_post();
        if ($custom_query->current_post > $args['pointer'] && $custom_query->current_post < $args['postLimit']) {
            ...
            $post_format = $postClass[4];
            $post_format = str_replace('format-','',$post_format);
            ...
            ?>
            <!-- // START feed-item removed from feed item class: first clearfix -->
            <div class="feed-item <?php echo $post_type . " " . $category ." ";?> <?php if($category!='quote'){ echo $post_format;}?> clearfix"
                 id="divID_<?php echo $post->ID; ?>">
                <?php if (has_post_thumbnail($custom_query->ID)) {
                    echo '<a href="' . get_permalink($custom_query->ID) . '" class="feed-link" title="' . esc_attr($custom_query->post_title) . '">';
                ...
                <div class="<?php if (!has_post_thumbnail($custom_query->ID)) {
                    echo 'expand-info';
                } else {
                    echo 'feed-info';
                } ?>">
                    ...
                    <div class="tags">
                        <?php  echo get_custom_tag(array('ID' => $post->ID, 'key' => 'name'));?>
                    </div>
                </div>
            </div>
            <!-- // end feed-item -->
            ...
            <div class="feed-item first clearfix" id="divID_<?php echo $post->ID; ?>">
                ...
                <div class="create-header">
                    <a href="<?php echo get_permalink($custom_query->ID) ?>"
                       title="<?php echo esc_attr($custom_query->post_title) ?>"><h2><?php the_title(); ?></h2></a>
                ...
                <div class="create-content">
                    <div class="create-info">
                        ...
                        <a href="<?php echo get_permalink($custom_query->ID) ?>"
                           title="<?php echo esc_attr($custom_query->post_title) ?>">
                            <h3>Challanges</h3>
                ...
                <div class="work-content first clearfix">
                    ...
                    <div class="work-metadata">
                        <div class="tags first clearfix">
                            ...
                            <p><?php get_connected_cases($post_connected_cases); ?></p>
                        </div>
                    </div>
                </div>
            </div> <!-- // end feed-item -->
...
function get_live_feed_model($args)
{
    ...
    while ($custom_query->have_posts()) : $custom_query->the_post();
        if ($custom_query->current_post > $args['pointer'] && $custom_query->current_post < $args['postLimit']) {
            ...
            ?>
            <!-- // START feed-item -->
            <div class="feed-item <?php echo $post_type . " " . $category ?> clearfix"
                 id="divID_<?php echo $post->ID; ?>">
                <?php if (has_post_thumbnail($custom_query->ID)) {
                    echo '<a href="' . get_permalink($custom_query->ID) . '" class="feed-link" title="' . esc_attr($custom_query->post_title) . '">';
                    echo '<div class="img-container">';
        ...
        if ($custom_query->current_post > $args['pointer'] && $custom_query->current_post < $args['postLimit']) {
            ...
            <div id="top-item">
                <div class="top-img-container">
                    ...
                    <div id="<?php if (!has_post_thumbnail($custom_query->ID)) {
                        echo 'expand-top-info';
                    } else {
                        echo 'top-info';
                    } ?>">
                        ...
                        <?php echo '</a>'; ?>
                        <div
                            id="tag-cloud"><?php echo get_custom_tag(array('ID' => $post->ID, 'key' => 'name')); ?></div>
                    </div>
                </div>
            </div>
            <?php
...
function get_connected_cases($arr)
{
    ...
    foreach ($arr as &$value) {
        if (isset($value)) {
            ...
            if ($count < $max) {
                ?><a
                href="<?php echo esc_url(get_permalink($value)); ?>"><?php echo $_post->post_title; ?></a>,&nbsp;<?php
            } else if ($count === $max) {
